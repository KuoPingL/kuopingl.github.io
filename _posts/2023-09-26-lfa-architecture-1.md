...
|Method|Purpose|
|:--|:--|
|`replace`   | 以 `OP_REPLACE` 調用 `doAppOp`。 <br><br>後來會通過 `BackStackRecord.expandOps` 將舊的 fragment 移除並將新的加上去。 <br><br>這行為並不會有動畫。 |
|`add`   | 以 `OP_ADD` 調用 `doAppOp`。<br><br>後來會通過 `FragmentManager.expandOps` 設定動畫並將 fragment 加在 `mManager` 中。 <br><br>當它被 pop 時，就會從 `mManager` 移除。 |
|`show`   |  以 `OP_SHOW` 調用 `doAddOp`。 <br><br>這會讓 `mManager` 直接調用 `FragmentManager.showFragment`。  |
|`hide`   | 以 `OP_HIDE` 調用 `doAddOp`。 <br><br>這會讓 `mManager` 直接調用 `FragmentManager.hideFragment`。 <br><br>這個功能並不會啟動動畫。  |
|`attach`   | 以 `OP_ATTACH` 跳過 `doAddOp` 並調用 `addOp` 並將 Op 放在 `mOps` 中。<br><br>最後設定了 `mManager` 的動畫，再調用 `FragmentManager.attachFragment`。 <br><br>這功能在加上時並不會啟動動畫，但 `detach` 時就會有動畫。  |
|`detach`   | 以 `OP_DETTACH` 調用 `addOp` 並將 Op 放在 `mOps` 中。 <br><br>這功能在 `expandOps` 會顯示動畫，但 `expandPopOps` 時則會調用 `FragmentManager.attachFragment`。  |
|`setPrimaryNavigationFragment`   | 以 `OP_SET_PRIMARY_NAV` 調用 `addOp`。<br><br>在 `expandOps` 時調用 `FragmentManager.setPrimaryNavigationFragment(f)` 若在 `expandPopOps` 則會將 `f` 設為 `null`。    |
|`setMaxLifecycle`   | 以 `OP_SET_MAX_LIFECYCLE` 調用 `addOp`。<br><br>在 `expandOps` 中設定 `f.mMaxState`。而在 `expandPopOps` 時就會將 `f.mMaxState` 設為 `mOldMaxState`。   |
